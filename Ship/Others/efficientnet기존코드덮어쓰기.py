#이 코드를 기존 efficientnet 소스코드에 통째로 복사 붙혀넣기 하시면 됩니다
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        self.classifier2 = nn.Sequential(
            nn.Dropout(p=dropout, inplace=False),
            nn.Linear(lastconv_output_channels, num_classes2),
        )
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        main_block = self.classifier(x)
        unit_block = self.classifier2(x)

        return main_block, unit_block

    def forward(self, x: Tensor) -> Tensor:
        main_block, unit_block = self._forward_impl(x)
        return main_block, unit_block
